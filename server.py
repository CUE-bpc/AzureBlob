...
# Azure hat mit cv2 ein Problem, daher gibt es ein Startup Command, siehe cuehelloworld Web App -> Settings -> Config
import cv2
import pandas as pd
import numpy as np
...
def upload_ocr():
    ...
    img = cv2.imdecode(np.array(bytearray(base64.b64decode(base_64_string_data)), dtype='uint8'), cv2.IMREAD_COLOR)
    #img = cv2.imdecode(np.array(bytearray(base_64_string_data), dtype='uint8'), cv2.IMREAD_COLOR)

    data = base_64_string_data
    ...
    if not computer_vision_resp['readResult']['content']:
        return jsonify({"error": "No meter reading detected."}), 400
